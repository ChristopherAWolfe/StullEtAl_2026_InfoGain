...
# information gain values into a .RDS file. DATA and VARIABLE INFORMATION must
...
var_info <-  load_var_info('data/US_allvar_var_info.csv')
...
## Step 6: Solve for X
### This step is necessary to solve for information gain over X (age)
...
### The model selection and IG calculation is completed in one step.
### The continuous variables utilize a secondary KL Divergence function.

####  Ord IG Function
calc_univ_ord_ig <- function(th_v, mod_spec, th_x, x0, xcalc, model) {
  # Calculate the information gain for a univariate ordinal model given the
...
#### Cont IG function
calc_univ_cont_ig <- function(th_w, mod_spec, x0, wcalc, kl_div) {
  # Calculate the information gain for a univariate cont. model given the
...
IG_ord <- setNames(vector("list", j_ord), main_problem$var_names[1:44])

# Loop over each ord var, select the best model,print best model,do IG based on
#best model and store results in IG_ord
...
clusterEvalQ(cl, {
  library(foreach)
  library(doParallel)
  library(yada)      # <-- also add packages your workers need!
})
...
for (i in 1:j_ord) {
  ...
  IG_ord[[i]] <- foreach(n=1:length(x0), .multicombine = T,
...
IG_cont <- setNames(vector("list", k_cont), main_problem$var_names[45:62])

##### Loop over each cont var, select the best model with aic, print best model,  
#do KL div,do IG based on best model and kl results and store results in IG_cont
for (i in 1:k_cont) {
  ...
  IG_cont[[i]] <- foreach(n=1:length(x0), .multicombine = T,
                          .packages = c("yada")) %dopar%{
...
write_rds(IG_ord, "results/IG_ord_allVars.rds")
write_rds(IG_cont,"results/IG_cont_allvars.rds")
